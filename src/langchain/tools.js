...
const prompt = ChatPromptTemplate.fromMessages([
    ...
    // Uncomment the following line if you wish to include chat history:
...
const shouldContinue = (state) => {
    ...
    if (state.agent_outcome && Array.isArray(state.agent_outcome) && state.agent_outcome[0]?.tool) {
...
// Instead of calling setEntryPoint separately, specify the entry point in the constructor
const workflow = new StateGraph({
    channels: agentState, // Define the structure of the state
    entry: "agent",       // New way to define the entry node
});
...
// Define conditional edges from the agent node: either continue to tool or end
...
export function formatFinalResponse(finalState) {
    ...
    } else if (typeof finalState?.agent_outcome === 'string') {
